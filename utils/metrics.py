...
def sliding_window_inference(net, image, patch_size, stride_xy, stride_z, n_classes):
    ...
    image: numpy array (W, H, D), already normalised.
    ...
    score_map = np.zeros((n_classes,) + image.shape, dtype=np.float32)
...
@torch.no_grad()
def evaluate(net, test_loader, patch_size, stride_xy=16, stride_z=4, n_classes=2):
    """
    Run sliding-window inference on all test volumes and return mean metrics.
    test_loader must yield (image_tensor, label_tensor) with batch_size=1.
    """
    dice_list, jc_list, hd_list, asd_list = [], [], [], []
    net.eval()

    for image, label in test_loader:
        image_np = image.squeeze().numpy()      # (W, H, D)
