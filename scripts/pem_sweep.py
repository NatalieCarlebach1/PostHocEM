"""
PEM hyperparameter sweep across label fractions.

For each (fraction, lr, mode) configuration, runs train_posthoc_em.py and
collects the best Dice. Skips already-completed runs based on metrics.csv.
At the end, prints a summary table and writes summary.csv.

Usage:
    python scripts/pem_sweep.py                  # full sweep
    python scripts/pem_sweep.py --fractions 20   # only 20% fraction
    python scripts/pem_sweep.py --dry_run        # print commands, don't run
"""
...
DEFAULT_FRACTIONS = [5, 10, 20, 40, 60, 80]

# Learning rates to try, from very gentle to aggressive
DEFAULT_LRS = [1e-6, 5e-6, 1e-5, 5e-5, 1e-4]

# Many confidence thresholds — covers the full spectrum
# Lower thresholds → fewer voxels masked → more conservative
DEFAULT_THRESHOLDS = [0.5, 0.7, 0.8, 0.9, 0.95, 0.99, 0.995, 0.999, 0.9999]
...
def run_name(fraction, lr, mode, threshold):
    ...
    if mode == 'confident':
        return f'pem_sweep_{fraction}pct_conf_t{threshold}_lr{lr_str}'
    return f'pem_sweep_{fraction}pct_full_lr{lr_str}'
...
def get_checkpoint_path(root, fraction):
    """Resolve the BCP checkpoint for a given label fraction."""
    if fraction == 20:
        return root / 'result' / 'bcp_baseline_v2' / 'best_model.pth'
    return root / 'result' / f'bcp_{fraction}pct' / 'best_model.pth'


def seed_baseline_cache(root, fractions, splits_dir, patch_size=96, num_classes=2):
    """Seed the baseline cache JSON next to each checkpoint, using the metrics
    already saved during BCP training. Avoids re-running baseline eval."""
    for fraction in fractions:
        ckpt_path = get_checkpoint_path(root, fraction)
        if not ckpt_path.exists():
            print(f'  [{fraction}%] checkpoint missing: {ckpt_path}')
            continue
        ...
        # Pull the row matching the best_dice from the BCP training CSV.
        bcp_csv = ckpt_path.parent / 'metrics.csv'
        if not bcp_csv.exists():
            print(f'  [{fraction}%] BCP metrics.csv missing — will compute on first run')
            continue

        with open(bcp_csv) as f:
            rows = list(csv.DictReader(f))
        if not rows:
            continue

        # Find the row with the highest dice (matches best_model.pth)
        best_row = max(rows, key=lambda r: float(r['dice']))
        cache_payload = {
            'key': {
            ...
            'metrics': {
                ...
                'jaccard': float(best_row['jaccard']),
                'hd95':    float(best_row['hd95']),
                'asd':     float(best_row['asd']),
            },
            'source': str(bcp_csv.relative_to(root)),
            'epoch':  best_row.get('epoch', ''),
        }
        cache_path.write_text(json.dumps(cache_payload, indent=2))
        print(f'  [{fraction}%] cached baseline {cache_payload["metrics"]["dice"]:.4f} '
              f'(from epoch {cache_payload["epoch"]})')


def run_one(fraction, lr, mode, threshold, epochs, root, dry_run=False):
    ...
    if res.returncode != 0:
        tail = '\n'.join(res.stderr.splitlines()[-3:]) if res.stderr else 'no stderr'
        print(f'  FAILED:\n{tail}')
...
def main():
    ...
    p.add_argument('--thresholds',type=float, nargs='*', default=DEFAULT_THRESHOLDS,
                   help='Confidence thresholds to sweep. Pass --thresholds (no values) to skip threshold sweep.')
    p.add_argument('--epochs',    type=int, default=DEFAULT_EPOCHS)
    p.add_argument('--include_full', action='store_true', default=True,
                   help='Also run full-entropy mode (default: True)')
    ...
    root = Path(args.root)
    summary_path = root / 'result' / 'pem_sweep_summary.csv'
    log_path = root / 'result' / 'pem_sweep.log'
    summary_path.parent.mkdir(parents=True, exist_ok=True)

    # ── Sweep logger: writes to both stdout and a persistent log file ───────
    ...
    # Build list of (mode, threshold) tuples
    ...
    # Seed baseline cache from BCP training CSVs (avoids re-running baseline eval)
    log('Seeding baseline cache from existing BCP metrics.csv files...')
    seed_baseline_cache(root, args.fractions, root / 'splits' / 'pancreas')

    n_total = len(args.fractions) * len(args.lrs) * len(mode_threshold_pairs)
    est_minutes = n_total * 3  # rough: ~3 min per run
    log('=' * 80)
    log(f'SWEEP START — log file: {log_path}')
    ...
    for fraction in args.fractions:
        # Check checkpoint exists for this fraction
        ...
        for lr in args.lrs:
            for mode, threshold in mode_threshold_pairs:
                ...
                # Write incremental summary after every run
                _write_summary(summary_path, rows)

    # ─── Final summary ──────────────────────────────────────────────────────
    log('=' * 80)
    log('BEST CONFIGURATION PER LABEL FRACTION')
    log('=' * 80)
